...
def stack_graph(g):
	'''
	(graph) => None
	create a subgraph of g. This has all nodes of g, and exactly one (or zero) weighted edge from n1 to n2.
	If g has n>1 edges from n1 to n2, the stacked subgraph has only one edge of weight n. 
	Also copy the original g into another unstacked subgraph, to preserve it.
	'''
	# initialize properties I need
	numComms = g.getIntegerProperty('numComms')
	# copy the parallel edges graph onto a new subgrah
	nonStacked = g.addSubGraph('nonStacked')
	for n in g.getNodes():
...
## stack all subgraph of the main graph:
	
	success = stack_graph(graph)

#	for g in graph.getSubGraphs():
#		success = stack_graph(g)
#
